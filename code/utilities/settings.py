# Settings

import pygame
from os.path import join

# Display Size
SCREEN_WIDTH = 1920  # 1920 1600
SCREEN_HEIGHT = 1280  # 1280 900
TILE_SIZE = 64
ANIMATION_SPEED = 6

# Player Settings
PLAYER_IMAGE = pygame.image.load(join('images', 'player.png'))
PLAYER_EXPERIENCE = 100
PLAYER_EXPERIENCE_NEED = 100
PLAYER_SPEED = 2
PLAYER_ATTACK_SPEED = 1
PLAYER_DAMAGE = 5
PLAYER_HEALTH = 10
PLAYER_MAX_HEALTH = 10
PLAYER_RANGED_DAMAGE = 0
PLAYER_MELEE_DAMAGE = 0
PLAYER_ENGINEER_DAMAGE = 0

# Background Settings
BACKGROUND_IMAGE = pygame.image.load(join('images', 'background_placeholder.png'))

# Template enemy settings
ENEMY_TEMPLATE_IMAGE = pygame.image.load(join('images', 'enemy.png'))
ENEMY_TEMPLATE_HEALTH = 10

# Bullet template settings
BULLET_TEMPLATE_IMAGE = pygame.image.load(join('images', 'bullet_template.png'))
BULLET_SPEED = 10

# UI Bar's settings
BAR_TRANSITION_SPEED = 0.1
